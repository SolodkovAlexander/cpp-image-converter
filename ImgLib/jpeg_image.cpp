...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
    ...
    std::vector<JSAMPLE> buffer(row_stride, 0);
    while (cinfo.next_scanline < cinfo.image_height) {
        auto row = image.GetLine(cinfo.next_scanline);
        for (auto i = 0; i < image.GetWidth(); ++i) {
            buffer[i*3] = static_cast<JSAMPLE>(row[i].r);
            buffer[i*3+1] = static_cast<JSAMPLE>(row[i].g);
            buffer[i*3+2] = static_cast<JSAMPLE>(row[i].b);
        }
